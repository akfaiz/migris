...
func (s *mysqlBuilderSuite) SetupSuite() {
	...
	container, db, err := testutil.StartMySQLTestDB(s.ctx)
...
func (s *mysqlBuilderSuite) TestDrop() {
	...
	s.Run("when all parameters are valid, should drop table successfully", func() {
		_ = builder.DropIfExists(c, "users_drop_test")
		err = builder.Create(c, "users_drop_test", func(table *schema.Blueprint) {
			table.ID()
			table.String("name", 255)
			table.String("email", 255).Unique()
			table.String("password", 255).Nullable()
		...
		err = builder.Drop(c, "users_drop_test")
